...
@Component({
  selector: 'app-your-activity-admin',
  standalone: true,
...
export class YourActivityComponent implements OnInit {
  ...
  fetchAdminActivity(): void {
    this.http.get<any[]>('https://localhost:7284/api/AdminActivity/GetAll')
      .subscribe({
        next: (data: any[]) => this.activityList = data,
        error: (err: any) => console.error('Error fetching admin activity:', err)
      });
  }

  downloadPDF(): void {
    const doc = new jsPDF();
    doc.text('Admin Login Activity Report', 14, 15);
    ...
    doc.save('Admin_Login_Activity.pdf');
  }
}
